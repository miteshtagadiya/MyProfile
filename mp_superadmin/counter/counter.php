...
// ################################################
	...
	$dbselect=mysql_select_db('myprofile'); 
	if (!$dbselect) 
	{
...
// ########################################################
	$data=mysql_query("SELECT page FROM mp_tablehits WHERE page = '$page'");
	if(mysql_num_rows($data))
	{
	//A counter for this page  already exsists. Now we have to update it.

		$updatecounter = mysql_query("UPDATE mp_tablehits SET count = count+1 WHERE page = '$page'");
		if (!$updatecounter) 
		{
	...
	// This page did not exsist in the counter database. A new counter must be created for this page.
		...
		$insert = mysql_query("INSERT INTO mp_tablehits (page, count)VALUES ('$page', '1')");
	
		if (!$insert) 
		{
    		die ("Can\'t insert into mp_tablehits : " . mysql_error()); // remove ?
		}
	}
...
if(!mysql_num_rows(mysql_query("SELECT ip_address FROM tableinfo WHERE ip_address = '$ip'"))) // check if the IP is in database
{
	// if not , add it.	
	$adddata = mysql_query("INSERT INTO tableinfo (ip_address, user_agent, datetime) VALUES('$ip' , '$agent','$datetime' ) ") ;
	if (!$adddata) 
	{
...
$result = mysql_query("SELECT * FROM tableinfo", $link);
...
if($to_delete > 0)
{
	for ($i = 1; $i <= $to_delete; $i++) 
	{
		...
		$delete = mysql_query("DELETE FROM tableinfo ORDER BY id LIMIT 1") ;
		if (!$delete) 
		{
